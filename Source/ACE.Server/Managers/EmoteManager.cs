        ...
        public void ExecuteEmote(BiotaPropertiesEmote emote, BiotaPropertiesEmoteAction emoteAction, ActionChain actionChain, WorldObject sourceObject = null, WorldObject targetObject = null)
        {
            ...
            switch ((EmoteType)emoteAction.Type)
            {
                ...
                case EmoteType.Goto:
                    ...
                    Console.WriteLine("The emote set ID is: " + result2.FirstOrDefault().EmoteSetId);  ///Got a null value form this line
                    ...
                    Console.WriteLine("The rng value was: " + rng2);
                    foreach (var action in actions2)
                    {
                        actionChain.AddAction(player, () =>
                        {
                            ExecuteEmote(result2.FirstOrDefault(), action, actionChain, sourceObject, player);
                            Console.WriteLine("This action is being added (from Goto): " + action.Message);
                        });
                    }
                    break;
                ...
                case EmoteType.InqAttributeStat:
                    ...
                    if (player != null)
                    {
                        ...
                        if (success)
                        {
                            ...
                            if (result.Count() > 0)
                            {
                                Console.WriteLine("Should be queueing emoteAactions from InqAttributeStat success.");
                                ...
                                foreach (var action in actions)
                                {
                                    actionChain.AddAction(player, () =>
                                    {
                                        ...
                                        Console.WriteLine("This action is being added (from inqAttributeStat success): " + action.Message);
                                    });
                                }
                            }
                        }
                        ...
                        {
                            Console.WriteLine("You do not meet the inqAttributeStat requirements");
                            ...
                            if (result.Count() > 0)
                            {
                                ...
                                foreach (var action in actions)
                                {
                                    ...
                                    {
                                        ExecuteEmote(result.FirstOrDefault(), action, actionChain, sourceObject, player);
                                        Console.WriteLine("This action is being added (from inqAttributeStat failure): " + action.Message);
                                    });
                                }
                            }
                        }
                ...
                case EmoteType.InqEvent:
                    ...
                    if (started)
                    {
                        ...
                        if (result.Count() > 0)
                        {
                            Console.WriteLine("Should be queueing emoteAactions from Event Success.");
                            var actions = sourceObject.Biota.BiotaPropertiesEmoteAction.Where(action => action.EmoteSetId == result.FirstOrDefault().EmoteSetId && action.EmoteCategory == result.FirstOrDefault().Category);
                            ...
                            foreach (var action in actions)
                            {
                                ...
                                {
                                    ExecuteEmote(result.FirstOrDefault(), action, actionChain, sourceObject, player);
                                    Console.WriteLine("This action is being added (from Event success): " + action.Message);
                                });
                            }
                        }
                    }
                    else
                    {
                        ...
                        if (result.Count() > 0)
                        {
                            ...
                            foreach (var action in actions)
                            {
                                ...
                                {
                                    ExecuteEmote(result.FirstOrDefault(), action, actionChain, sourceObject, player);
                                    Console.WriteLine("This action is being added (from Event failure): " + action.Message);
                                });
                            }
                        }
                    }
                ...
                case EmoteType.LocalBroadcast:
                    ...
                    text = emoteAction.Message;
                    WorldObject.CurrentLandblock.EnqueueBroadcastSystemChat(targetObject, text, ChatMessageType.Broadcast);
                    break;
